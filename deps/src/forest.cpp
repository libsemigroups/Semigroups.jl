...
namespace jlcxx {
template <> struct IsMirroredType<libsemigroups::Forest> : std::false_type {};
}    // namespace jlcxx
...
void define_forest(jl::Module & m)
{
  ...
  m.method("Forest", [](size_t n) -> Forest {
    return Forest(n);
  });
  ...
  type.method("forest_is_not_equal", [](Forest const & a, Forest const & b) -> bool {
    return a != b;
  });
  type.method("forest_is_equal", [](Forest const & a, Forest const & b) -> bool {
    return a == b;
  });
  ...
  type.method("forest_to_human_readable_repr", [](Forest const & f) {
    return to_human_readable_repr(f);
  });
