...
namespace libsemigroups_julia {
  ...
  namespace {
    ...
    void bind_ptransf_common(jl::Module&                      m,
      ...
      // Pure pass-through. Julia passes 0-based indices with typemax(T)
      // as the UNDEFINED sentinel — matches libsemigroups' native format.
      ...
      type.method("copy",
                  [](PTransfType const& self) { return PTransfType(self); });

      // __getitem__ equivalent - element access with bounds checking.
      // Julia passes 0-based index; returns raw libsemigroups value
      // (typemax(T) for undefined points on PPerm).
      ...
      // increase_degree_by - modifies in place, don't expose base class return
      // type
      type.method("increase_degree_by!", [](PTransfType& self, size_t m) {
        self.increase_degree_by(m);
      });

      // Raw image vector (0-based, typemax(T) for undefined points).
      type.method("images_vector", [](PTransfType const& self) {
        return std::vector<Scalar>(self.begin(), self.end());
      });
    ...
    void bind_pperm_type(jl::Module& m, std::string const& name) {
      ...
      // Julia passes 0-based domain/image vectors; degree is a count.
      m.method(name,
               [](std::vector<Scalar> const& dom,
                  std::vector<Scalar> const& img,
    ...
    void bind_perm_type(jl::Module& m, std::string const& name) {
  ...
  void define_transf(jl::Module& m) {
    ...
    m.method("one",
             [](Transf<0, uint8_t> const& f) { return libsemigroups::one(f); });
    ...
    m.method("one",
             [](PPerm<0, uint8_t> const& f) { return libsemigroups::one(f); });
    m.method("one",
             [](PPerm<0, uint16_t> const& f) { return libsemigroups::one(f); });
    m.method("one",
             [](PPerm<0, uint32_t> const& f) { return libsemigroups::one(f); });

    m.method("one",
             [](Perm<0, uint8_t> const& f) { return libsemigroups::one(f); });
    m.method("one",
             [](Perm<0, uint16_t> const& f) { return libsemigroups::one(f); });
    m.method("one",
             [](Perm<0, uint32_t> const& f) { return libsemigroups::one(f); });
    ...
    // domain(f) - returns raw libsemigroups::domain (0-based indices).
